    ...
    public class CityDataSeederService
    {
        ...
        private async Task<List<ArgentineCityDto>> LoadCitiesFromJsonFile()
        {
            try
            {
                ...
                if (stream == null)
                {
                    _logger.LogWarning("⚠️ ArgentineCities.json file not found as embedded resource: {ResourceName}", resourceName);
                    return new List<ArgentineCityDto>();
                }
                ...
                var cities = JsonSerializer.Deserialize<List<ArgentineCityDto>>(jsonContent, options);
                return cities ?? new List<ArgentineCityDto>();
            }
            catch (Exception ex)
            {
        ...
        private async Task ProcessAndInsertCities(List<ArgentineCityDto> argentinaCities, List<State> states)
        {
